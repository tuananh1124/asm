...
-- tạo table để chat realtime
CREATE TABLE messages
(
    message_id   INT PRIMARY KEY IDENTITY(1,1),  -- ID tự động tăng cho mỗi tin nhắn
    sender       VARCHAR(50) NOT NULL,           -- Username của người gửi
    receiver     VARCHAR(50) NOT NULL,           -- Username của người nhận
    content      NVARCHAR(MAX) NOT NULL,         -- Nội dung tin nhắn
    sent_at      DATETIME DEFAULT GETDATE(),     -- Thời gian gửi tin nhắn
    is_read      BIT DEFAULT 0,                  -- Trạng thái đã đọc hay chưa

    -- Khóa ngoại liên kết với bảng accounts
    CONSTRAINT FK_messages_sender FOREIGN KEY (sender) REFERENCES accounts(username),
    CONSTRAINT FK_messages_receiver FOREIGN KEY (receiver) REFERENCES accounts(username)
);
GO
